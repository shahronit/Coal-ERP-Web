...
This repo includes a [`render.yaml`](render.yaml) Blueprint (PostgreSQL + Docker web service in **Singapore**).

1. Push this repo to GitHub (already at [Coal-ERP-Web](https://github.com/shahronit/Coal-ERP-Web)).
2. Open the [Render Blueprint deploy link](https://dashboard.render.com/blueprint/new?repo=https://github.com/shahronit/Coal-ERP-Web).
3. Connect your GitHub account if prompted, review the two resources (`coal-erp-db`, `coal-erp-web`), and click **Apply**.
4. Wait for the first deploy (~5–10 min). JWT secrets are auto-generated by Render.

**Live URL:** `https://coal-erp-web.onrender.com` (or the name Render assigns).

**Default login** (created on first deploy): `superadmin@tradecrm.com` / `Demo@123`
